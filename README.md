...
0. `ocommit`
1. Shows the current `git status` and generates a commit message based off the
   diff 
...
git clone https://github.com/bogzbonny/ocommit
cd ocommit

# Build with Cargo (requires a recent Rust toolchain)
cargo install --path .
```
...
- Cargo.lock                     #   to what's in the .gitignore)
```
